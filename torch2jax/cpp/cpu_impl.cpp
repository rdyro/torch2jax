...
void cpu_apply_torch_call(void *out_tuple, const void **in) {
  ...
  // int64_t counter = 0;
  // DynamicTorchCallDescriptor d = deserialize_cpu_descriptor(in, &counter);
  // in += counter;

  DescriptorDataAccessor data(in, nullptr);
  DynamicTorchCallDescriptor d;
  int64_t k = deserialize_descriptor(d, data);
  in += k;
